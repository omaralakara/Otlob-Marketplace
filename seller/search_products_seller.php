...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['search_term'])) {
    ...
    // Fetch products based on search query for the logged-in seller
    ...
    $totalProductsQuery = $conn->query("SELECT COUNT(*) as total FROM products WHERE 
                                        seller_id = $sellerID AND
                                        (product_name LIKE '%$searchTerm%' OR
                                         product_id LIKE '%$searchTerm%')");
    ...
    // If no search term is provided, fetch all products for the logged-in seller
...
<div class="pagination" style="margin-top: 20px;">
    <?php if ($page > 1): ?>
        <a href="?page=<?php echo ($page - 1); ?>&search_term=<?php echo urlencode($searchTerm); ?>" class="page-link">Previous</a>
    <?php endif; ?>
    
    <?php for ($i = 1; $i <= $totalPages; $i++): ?>
        <a href="?page=<?php echo $i; ?>&search_term=<?php echo urlencode($searchTerm); ?>" class="page-link <?php echo ($page == $i) ? 'active' : ''; ?>"><?php echo $i; ?></a>
    <?php endfor; ?>

    <?php if ($page < $totalPages): ?>
        <a href="?page=<?php echo ($page + 1); ?>&search_term=<?php echo urlencode($searchTerm); ?>" class="page-link">Next</a>
    <?php endif; ?>
</div>
