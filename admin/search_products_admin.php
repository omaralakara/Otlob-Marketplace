...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['search_term'])) {
    ...
    // Fetch products based on search query (product name, ID, category name, or store name)
    $query = "SELECT p.*, s.store_name 
              FROM products p
              LEFT JOIN sellers s ON p.seller_id = s.seller_id
              WHERE 
              (p.product_name LIKE '%$searchTerm%' OR
               p.product_id LIKE '%$searchTerm%' OR
               p.category_name LIKE '%$searchTerm%' OR
               s.store_name LIKE '%$searchTerm%')
              LIMIT $start, $limit";

    $productsQuery = $conn->query($query);
    $products = $productsQuery->fetch_all(MYSQLI_ASSOC);

    // Fetch total number of products matching the search query
    $totalProductsQuery = $conn->query("SELECT COUNT(*) as total 
                                        FROM products p
                                        LEFT JOIN sellers s ON p.seller_id = s.seller_id
                                        WHERE 
                                        (p.product_name LIKE '%$searchTerm%' OR
    ...
    if ($totalProducts == 0) {
        echo '<div class="alert alert-warning" role="alert">No product matching the search term found.</div>';
    }
} else {
    ...
    $productsQuery = $conn->query("SELECT p.*, s.store_name 
                                   FROM products p
                                   LEFT JOIN sellers s ON p.seller_id = s.seller_id
                                   LIMIT $start, $limit");
    $products = $productsQuery->fetch_all(MYSQLI_ASSOC);
    ...
    <div class="row">
        ...
        <div class="col-lg-3">
            <div class="list-group">
                ...
                <a href="view_products.php" class="list-group-item list-group-item-action active">View Products</a>
            </div>
        </div>
...
<div class="col-lg-9">
    ...
    <h2>Product List</h2>
       ...
       <!-- Search Form -->
       <form action="search_products_admin.php" method="GET" class="mb-3">
        ...
        <table class="table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?php echo $product['store_name']; ?></td> <!-- Corrected line to display store name -->
...
<div class="pagination" style="margin-top: 20px;">
    <?php if ($page > 1): ?>
        <a href="?page=<?php echo ($page - 1); ?>&search_term=<?php echo urlencode($_GET['search_term']); ?>" class="page-link">Previous</a>
    <?php endif; ?>
    
    <?php for ($i = 1; $i <= $totalPages; $i++): ?>
        <a href="?page=<?php echo $i; ?>&search_term=<?php echo urlencode($_GET['search_term']); ?>" class="page-link <?php echo ($page == $i) ? 'active' : ''; ?>"><?php echo $i; ?></a>
    <?php endfor; ?>

    <?php if ($page < $totalPages): ?>
        <a href="?page=<?php echo ($page + 1); ?>&search_term=<?php echo urlencode($_GET['search_term']); ?>" class="page-link">Next</a>
    <?php endif; ?>
</div>
