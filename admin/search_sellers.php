...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['search_term'])) {
    ...
    // Fetch sellers based on search query (seller name, store name, or email)
    $query = "SELECT * FROM sellers WHERE 
              seller_name LIKE '%$searchTerm%' OR
              store_name LIKE '%$searchTerm%' OR
              seller_email LIKE '%$searchTerm%'
              LIMIT $start, $limit";
    $sellersQuery = $conn->query($query);
    $sellers = $sellersQuery->fetch_all(MYSQLI_ASSOC);

    // Fetch total number of sellers matching the search query
    $totalSellersQuery = $conn->query("SELECT COUNT(*) as total FROM sellers WHERE 
                                        seller_name LIKE '%$searchTerm%' OR
                                        store_name LIKE '%$searchTerm%' OR
                                        seller_email LIKE '%$searchTerm%'");
    $totalSellers = $totalSellersQuery->fetch_assoc()['total'];

    // Display message if no seller is found
    if ($totalSellers == 0) {
        echo '<div class="alert alert-warning" role="alert">No seller matching the search term found.</div>';
    }
} else {
    // If no search term is provided, fetch all sellers
