...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['confirm_delete'])) {
        ...
        // Redirect to the view products page without deleting
        header("Location: view_products.php");
        exit();
    }
}
...
$query = "
    ...
    WHERE 
          p.product_id LIKE '%$search%' OR
          p.seller_id LIKE '%$search%' OR
          p.category_id LIKE '%$search%' OR
          p.product_name LIKE '%$search%' OR
          p.description LIKE '%$search%' OR
          p.price LIKE '%$search%' OR
          p.created_at LIKE '%$search%' OR
          p.category_name LIKE '%$search%' OR
          p.color1 LIKE '%$search%' OR
          p.color2 LIKE '%$search%' OR
          p.color3 LIKE '%$search%' OR
          p.size1 LIKE '%$search%' OR
          p.size2 LIKE '%$search%' OR
          p.size3 LIKE '%$search%' OR
          p.type LIKE '%$search%' OR
          p.brand LIKE '%$search%' OR
          p.counter LIKE '%$search%'
    LIMIT $start, $limit";
...
<div class="pagination" style="margin-top: 20px;">
    ...
    <?php for ($i = max(1, $page - 1); $i <= min($totalPages, $page + 1); $i++): ?>
        <?php if ($i == $page - 1 || $i == $page || $i == $page + 1): ?>
